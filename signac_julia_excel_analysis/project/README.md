...
All the `signac` commands are run from the `<local_path>/signac_julia_excel_analysis/signac_julia_excel_analysis/signac_julia_excel_analysis/project ` directory, which include, but are not limited to the following commands:
...
```bash
row submit --action part_2_julia_dot_product_calcs_command
```
...
```bash
row submit --action part_3_calc_avg_std_dev_command
```
